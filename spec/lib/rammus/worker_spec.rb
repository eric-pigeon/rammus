module Rammus
  RSpec.describe 'Workers', browser: true do
    ...
    it 'should emit created and destroyed events' do
      ...
      worker_object = await page.evaluate_handle_function '() => new Worker("data:text/javascript,1")'
      worker = await worker_created_promise
      worker_this = await worker.evaluate_handle_function '() => this'
      worker_destroyed_promise = Promise.new { |resolve, _| page.once :worker_destroyed, resolve }
      await page.evaluate_function 'workerObj => workerObj.terminate()', worker_object
    ...
    it 'should have JSHandles for console logs' do
      ...
      await page.evaluate_function "() => new Worker(`data:text/javascript,console.log(1,2,3,this)`)"
    ...
    it 'should have an execution context' do
      worker_created_promise = Promise.new { |resolve, _|  page.once :worker_created, resolve }
      await page.evaluate_function "() => new Worker(`data:text/javascript,console.log(1)`)"
      worker = await worker_created_promise
      expect(await worker.execution_context.evaluate '1+1').to eq 2
    end

    it 'should report errors' do
      error_promise = Promise.new { |resolve, _| page.on :page_error, resolve }
      await page.evaluate_function "() => new Worker(`data:text/javascript, throw new Error('this is my error');`)"
